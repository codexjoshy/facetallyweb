...
function DownloadButton({ type }) {
  const button = variants[type || 'android'] ?? variants['android'];
  return (
    <div>
      <a
        href={button?.url}
      ...
      >
        <svg
          ...
          viewBox="0 0 384 512"
        >
          <path fill="currentColor" d={button?.path}></path>
        </svg>
        <div className="text-left">
          <div className="mb-1 text-xs">{button?.text1}</div>
